...
class CoursesDetailView(TemplateView):
    ...
    def get_context_data(self, pk=None, **kwargs):
        ...
        cached_feedback = cache.get(f"feddback_list_{pk}")
        if not cached_feedback:
            ...
            cache.set(f"feddback_list_{pk}", context["feedback_list"], timeout=300)
            # import pickle
            # with open(
    ...
    def post(self, *args, **kwargs):
        if self.request.user.is_authenticated:
            ...
            if not cache_lock_flag:
                cache_lock_flag = cache.get(f"mail_feedback_lock_{self.request.user.pk}", "lock", timeout=300)
                messages.add_message(self.request, messages.INFO, _("Message sended"))
                main_tasks.send_feedback_mail.delay(
                    {
